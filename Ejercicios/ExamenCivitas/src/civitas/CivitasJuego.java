...
public class CivitasJuego {
    ...
    private void inicializaTablero(MazoSorpresas mazo){
        ...
        tablero.añadeCasilla(new CasillaCalleSostenible("Acera del Darro", 400, 250, 200));
        ...
        tablero.añadeCasilla(new CasillaCalleSostenible("Oficios", 1000, 750, 500));
    ...
    private void inicializaMazoSorpresas(){
        mazo.alMazo(new SorpresaPagarCobrar("Te han robado la tarjeta de credito"
                + "\n en el Almanjayar y pierdes 1000€ en una transferencia por una tele", -1000));
        mazo.alMazo(new SorpresaPagarCobrar("Te inspeccionan por monopolio y debes "
                + "\nsobornar al estado para no acabar en la cárcel.", -1500));
        ...
        mazo.alMazo(new SorpresaPorCasaHotel("La luz ha subido tanto que los inquilinos "
                + "\nno pagan.",-150));
    ...
    public boolean hacerCalleSostenible(int ip){
        ...
        CasillaCalleSostenible calleS=jugador.hacerCalleSostenible(ip);
        ArrayList<CasillaCalle> propiedades = jugador.getPropiedades();
        
        if(jugador.existeLaPropiedad(ip)){
            CasillaCalle calle = propiedades.get(ip);
            if(jugador.puedoConvertirCalle(calle)){
                calleS = calle.hacermeSostenible(jugador);
                if(calleS != null){
                    Diario.getInstance().ocurreEvento("La calle " + calle.getNombre() 
                        + " se ha convertido en una calle sostenible.");
                    propiedades.set(ip, calle);
                }
            }    
        }
        if(calleS != null){
