...
public class Jugador implements Comparable<Jugador>{
    ...
    CasillaCalleSostenible hacerCalleSostenible(int ip){
        CasillaCalle aconvertir = propiedades.get(ip);
        CasillaCalleSostenible convertida = null;
        if(puedoConvertirCalle(propiedades.get(ip))){
            convertida = new CasillaCalleSostenible(aconvertir.getNombre(),
                                                                           aconvertir.getPrecioCompra(),
                                                                           aconvertir.getPrecioEdificar(),
                                                                           aconvertir.getPrecioBaseAlquiler());
            propiedades.set(ip, convertida);
        }
        return convertida;
    }
    
    public boolean puedoConvertirCalle(CasillaCalle casilla){ //NO COINCIDE CON DIGRAMA
        if(saldo > casilla.getImporteHacermeSostenible())
            return true;
        return false;
    }
